...
import { Box, Button, Typography, TextField, MenuItem, Select, InputLabel, FormControl, Checkbox, ListItemText, OutlinedInput } from '@mui/material';
...
export default function OrderForm({ mode = 'new', studentDocId, orderId }) {
    ...
    if (mode === 'edit' && studentDocId && orderId) {
      (async () => {
        ...
        if (orderSnap.exists()) {
          ...
          setSelectedItems(data.menuItems ? data.menuItems.map(i => i.id) : []);
          setPickupOrDelivery(data.pickupOrDelivery || 'pickup');
          setDeliveryRoom(data.deliveryRoom || '');
          setRequiredTime(data.requiredTime ? new Date(data.requiredTime) : null);
          setNotes(data.notes || '');
  ...
  const isFormValid = () => {
    if (!requiredTime || selectedItems.length === 0 || !pickupOrDelivery) return false;
    ...
    <Box sx={{ maxWidth: 500, mx: 'auto', mt: 5, p: 3, bgcolor: 'background.default', color: 'text.primary', borderRadius: 2, boxShadow: 3 }}>
      ...
      <form onSubmit={handleFormSubmit}>
        <Autocomplete
          ...
          onChange={(_, newValue) => mode === 'edit' ? null : setStudentIdInput(newValue ? newValue.studentId : '')}
          renderInput={(params) => (
            <TextField
              {...params}
              label="Student ID"
        ...
        <FormControl fullWidth sx={{ mb: 2 }} disabled={isReadOnly}>
          <InputLabel id="menu-items-label"><ShoppingCartIcon fontSize="small" sx={{ mr: 1, verticalAlign: 'middle' }} />Menu Items</InputLabel>
          <Select
        ...
        <FormControl fullWidth sx={{ mb: 2 }} disabled={isReadOnly}>
          <InputLabel id="pickup-delivery-label"><DeliveryDiningIcon fontSize="small" sx={{ mr: 1, verticalAlign: 'middle' }} />Pickup or Delivery</InputLabel>
          <Select
        ...
        <TextField
          label={<span><NotesIcon fontSize="small" sx={{ mr: 1, verticalAlign: 'middle' }} />Notes</span>}
          value={notes}
